...
public class CompraControllerComplement {
    ...
    @PostMapping("/compra/{vendedorID}")
    public @ResponseBody ResponseEntity<CompraDTO> agregarCompraVendedor( @PathVariable("vendedorID") Long idVendedor, @RequestBody @Valid CompraDTO compra ) {
    ...
    @GetMapping("/compra")
    public @ResponseBody ResponseEntity< List<CompraDTO> > obtenerCompras() {
            List<CompraDTO> compras = repo_compra.findAll().stream().map( c -> new CompraDTO(c) ).collect(Collectors.toList());
